...
This script is used for incremental pre-training the library models for causal language modeling (GPT, GPT-2, CTRL, ...) on a text file or a dataset.

The script takes model arguments, data arguments, and script arguments as inputs. Model arguments specify the model type, model checkpoint, tokenizer, and other options. Data arguments specify the dataset to use, train and validation data file directories, and other options. Script arguments control the use of PEFT (Parallel Execution Fault Tolerance) and other options.

The script provides functions for computing accuracy metrics, preprocessing logits for metrics calculation, and fault tolerance data collation.

To use this script, specify the necessary arguments and run the script. The fine-tuned model can be used for various downstream tasks in natural language processing.
"""
...
def main():
    ...
    print("我们正在预训练模型，注入领域知识")
    print(f"模型参数: {model_args.model_type}")
    ...
    print(f'config_class: {config_class}, model_class: {model_class}, tokenizer_class: {tokenizer_class}')
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file_dir is not None and os.path.exists(data_args.train_file_dir):
            ...
            logger.info(f"train files: {train_data_files}")  #  ['./data/pretrain/en_article_tail500.txt', './data/pretrain/fever.txt', './data/pretrain/tianlongbabu.txt']
        ...
        if data_args.validation_file_dir is not None and os.path.exists(data_args.validation_file_dir):
            ...
            logger.info(f"eval files: {eval_data_files}") #  ['./data/pretrain/en_article_tail500.txt', './data/pretrain/fever.txt', './data/pretrain/tianlongbabu.txt']
    ...
    logger.info(f"Raw datasets: {raw_datasets}") # raw_datasets: {'train': ..., 'validation': Dataset(features: ...}
    ...
    with training_args.main_process_first(desc="Dataset tokenization and grouping"): # 这段代码的目的是根据配置条件，对原始数据集进行分词和（或）分组处理， 得到 lm_dataset
    ...
    if training_args.do_train:
        ...
        train_dataset = lm_datasets['train'] # train_dataset: Dataset(columns: ['attention_mask', 'input_ids', 'labels'])
        ...
        logger.debug(tokenizer.decode(train_dataset[0]['input_ids'])) # Tokenized training example: 查看输入文本的编码
    ...
    if model_args.model_type and model_args.model_name_or_path:
        ...
        ddp = world_size != 1
        if ddp: # 这段代码涉及到分布式训练的设置，特别是与 DDP（DistributedDataParallel）相关的设置
    ...
    if training_args.gradient_checkpointing: # 梯度检查点是一种用于减少神经网络训练过程中内存使用的技术。它通过在前向传递过程中保存中间计算结果，然后在反向传递过程中重新计算这些结果来实现。
        model.gradient_checkpointing_enable() # 使用梯度检查点的主要优势是能够在有限的 GPU 内存下训练更大的模型或使用更大的批量大小。但代价是会增加一些计算时间，通常会导致 20%-30% 的训练速度降低。
