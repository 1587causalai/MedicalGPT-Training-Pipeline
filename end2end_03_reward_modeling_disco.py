...
class BloomForGaussianRewardModeling(BloomPreTrainedModel):
    ...
    def forward(
        ...
        # log_var = torch.clamp(log_var, min=-20, max=2)  # Constrain log_var to avoid extreme values
        # var = torch.exp(log_var)
        var = torch.nn.functional.softplus(log_var)  # softplus to ensure variance is positive
...
def compute_metrics(eval_preds):
    rewards_chosen, rewards_rejected = eval_preds
    reward_accuracy = (rewards_chosen > rewards_rejected).mean().item()
    return {"reward_accuracy": reward_accuracy}
...
class DiscoRewardTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Assuming the model now outputs both mean and variance
        mu_chosen, var_chosen = rewards_chosen[0], rewards_chosen[1]
        mu_rejected, var_rejected = rewards_rejected[0], rewards_rejected[1]

        preference_prob = torch.special.ndtr(
            (mu_chosen - mu_rejected) / torch.sqrt(var_chosen + var_rejected)
        )

        # Compute the loss (negative log likelihood)
        loss = -torch.log(preference_prob).mean()

        if return_outputs:
            return loss, {"rewards_chosen": rewards_chosen, "rewards_rejected": rewards_rejected,
                          "log_var_chosen": log_var_chosen, "log_var_rejected": log_var_rejected}
        return loss
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        ...
        mean_chosen, variance_chosen = outputs_chosen.mean.detach(), outputs_chosen.variance.detach()
        ...
        mean_rejected, variance_rejected = outputs_rejected.mean.detach(), outputs_rejected.variance.detach()

        # Calculate the preference probability using the Disco Reward formula
        preference_prob = torch.special.ndtr(
            (mean_chosen - mean_rejected) / torch.sqrt(variance_chosen + variance_rejected)
        )
        ...
        return (loss, mean_chosen, mean_rejected)  # TODO: 需要仔细斟酌才能确定这样是否合理, 以及是否需要返回更多的信息
...
def main():
    ...
    if training_args.do_train:
        ...
        if trainer.is_world_process_zero():
            logger.debug(
                f"Number of train dataloader example: {batch['input_ids_chosen'].shape}"
            )
